...
suite("Minimap Decorations Test Suite", () => {
  ...
  test("Should create decoration types for all severity levels", () => {
    ...
    assert.strictEqual(
      decorationTypes.size,
      4,
      "Should create decoration types for 4 severity levels",
    );
    assert.ok(
      decorationTypes.has("critical"),
      "Should have critical severity decoration type",
    );
    assert.ok(
      decorationTypes.has("high"),
      "Should have high severity decoration type",
    );
    assert.ok(
      decorationTypes.has("medium"),
      "Should have medium severity decoration type",
    );
    assert.ok(
      decorationTypes.has("low"),
      "Should have low severity decoration type",
    );
  });
  ...
  test("Should not update decorations when minimap is disabled", () => {
    ...
    const decorations = minimapService.getActiveDecorationsForTesting(
      mockResult.filePath,
    );
    assert.strictEqual(
      decorations.length,
      0,
      "Should not create decorations when disabled",
    );
  ...
  test("Should create decorations only for minimap-enabled matchers", () => {
    ...
    assert.ok(
      ...
      "Should include high severity decoration",
    );
    assert.ok(
      severities.includes("medium"),
      "Should include medium severity decoration",
    );
    assert.ok(
      !severities.includes("low"),
      "Should not include low severity decoration (minimap: false)",
    );
  });
  ...
  test("Should create correct decoration ranges", () => {
    ...
    assert.ok(
      firstDecoration.range.isEqual(expectedRange),
      "Should create correct range for match",
    );
    assert.strictEqual(
      firstDecoration.severity,
      "high",
      "Should preserve severity",
    );
    assert.strictEqual(
      firstDecoration.matcherName,
      "Error Pattern",
      "Should preserve matcher name",
    );
  });
  ...
  test("Should clear decorations for specific file", () => {
    ...
    let decorations = minimapService.getActiveDecorationsForTesting(
      mockResult.filePath,
    );
    assert.ok(
      decorations.length > 0,
      "Should have decorations before clearing",
    );

    minimapService.clearDecorationsForFile(mockResult.filePath);
    decorations = minimapService.getActiveDecorationsForTesting(
      mockResult.filePath,
    );
    assert.strictEqual(
      decorations.length,
      0,
      "Should have no decorations after clearing",
    );
  });
  ...
  test("Should clear all decorations", () => {
    ...
    assert.ok(
      minimapService.getActiveDecorationsForTesting(mockResult1.filePath)
        .length > 0,
    );
    assert.ok(
      minimapService.getActiveDecorationsForTesting(mockResult2.filePath)
        .length > 0,
    );
  ...
  test("Should handle results with no minimap-enabled matchers", () => {
    ...
    assert.strictEqual(
      decorations.length,
      0,
      "Should handle results with no minimap-enabled matchers",
    );
  });
  ...
  test("Should create decorations with correct properties", () => {
    ...
    assert.strictEqual(
      decoration.matcherName,
      "Error Pattern",
      "Should have correct matcher name",
    );
    assert.strictEqual(
      decoration.message,
      "Found error pattern",
  ...
  test("Should update existing decorations when file is re-analyzed", () => {
    ...
    assert.strictEqual(
      decorations.length,
      2,
      "Should have initial decorations",
    );
    ...
    assert.strictEqual(
      decorations.length,
      1,
      "Should update to new decoration count",
    );
  });

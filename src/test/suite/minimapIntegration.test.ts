...
suite('Minimap Integration Test Suite', () => {
  ...
  setup(() => {
    outputChannel = vscode.window.createOutputChannel('Integration Test Output');
...
2024-01-01 10:05:00 DEBUG Processing request
    ...
    // Analyze the file
    const analysisResult = await patternMatcher.analyzeFile(tempFilePath, config);
    
    // Verify analysis results
    assert.ok(analysisResult.matches.length > 0, 'Should find matches in the log file');
    
    // Update minimap decorations
    minimapService.updateDecorations(analysisResult);
    
    // Verify minimap decorations
    const decorations = minimapService.getActiveDecorationsForTesting(tempFilePath);
    
    // Should have decorations for ERROR, WARN, and DEBUG (INFO has minimap: false)
    const expectedMinimapMatches = analysisResult.matches.filter(m => m.matcher.minimap);
    assert.strictEqual(
      decorations.length, 
    ...
    const warningDecorations = decorations.filter(d => d.severity === 'medium');
    
    assert.ok(errorDecorations.length >= 2, 'Should have at least 2 error decorations');
    assert.ok(warningDecorations.length >= 1, 'Should have at least 1 warning decoration');

    // Verify decoration ranges are valid
    for (const decoration of decorations) {
      assert.ok(decoration.range instanceof vscode.Range, 'Decoration should have valid range');
      assert.ok(decoration.range.start.line >= 0, 'Range should have valid start line');
      assert.ok(decoration.range.start.character >= 0, 'Range should have valid start character');
      assert.ok(decoration.color.length > 0, 'Decoration should have color');
      assert.ok(decoration.matcherName.length > 0, 'Decoration should have matcher name');
    }
  });
  ...
  test('Should handle configuration changes affecting minimap', async () => {
    ...
    // Analyze and update decorations
    let analysisResult = await patternMatcher.analyzeFile(tempFilePath, initialConfig);
    minimapService.updateDecorations(analysisResult);
    
    let decorations = minimapService.getActiveDecorationsForTesting(tempFilePath);
    assert.strictEqual(decorations.length, 1, 'Should have decoration when minimap is enabled');
    ...
    analysisResult = await patternMatcher.analyzeFile(tempFilePath, updatedConfig);
    minimapService.updateDecorations(analysisResult);
    
    decorations = minimapService.getActiveDecorationsForTesting(tempFilePath);
    assert.strictEqual(decorations.length, 0, 'Should have no decorations when minimap is disabled');
  });

  test('Should handle multiple files with different configurations', async () => {
    // Create two different log files
    const logFile1 = await createTempLogFile('ERROR: Database error\nWARN: Low memory');
    const logFile2 = await createTempLogFile('CRITICAL: System failure\nINFO: System restored');
    ...
    // Analyze both files
    const result1 = await patternMatcher.analyzeFile(logFile1, config1);
    const result2 = await patternMatcher.analyzeFile(logFile2, config2);
    ...
    const decorations1 = minimapService.getActiveDecorationsForTesting(logFile1);
    const decorations2 = minimapService.getActiveDecorationsForTesting(logFile2);
    ...
    assert.strictEqual(decorations1[0].severity, 'high', 'File 1 should have high severity decorations');
    assert.strictEqual(decorations2[0].severity, 'critical', 'File 2 should have critical severity decorations');
  });

  async function createTempLogFile(content: string): Promise<string> {
    const tempDir = require('os').tmpdir();
    const fileName = `test-log-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.log`;

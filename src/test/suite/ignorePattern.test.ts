...
suite("Ignore Pattern Tests", () => {
  ...
  suite("Pattern Matching with Ignore Pattern", () => {
    test("Should ignore matches when ignore pattern matches", () => {
      ...
      assert.strictEqual(
        allMatches.length,
        3,
        "Should find 3 matches (ignoring 2 flaky lines)",
      );
      assert.strictEqual(
        allMatches[0].line,
        1,
      ...
      assert.strictEqual(
        allMatches[2].line,
        5,
        "Third match should be on line 5",
      );
    });
    ...
    test("Should work without ignore pattern", () => {
      ...
      assert.strictEqual(
        allMatches.length,
        3,
        "Should find all 3 matches without ignore pattern",
      );
    });
    ...
    test("Should handle case sensitivity in ignore pattern", () => {
      ...
      assert.strictEqual(
        allMatches.length,
        1,
        "Should find 1 match (ignoring 3 flaky lines)",
      );
    ...
    test("Should handle case sensitivity when ignoreCase is false", () => {
      ...
      assert.strictEqual(
        allMatches.length,
        2,
        "Should find 2 matches (case sensitive)",
      );
      assert.strictEqual(
        allMatches[0].line,
        1,
      ...
      assert.strictEqual(
        allMatches[1].line,
        2,
        "Second match should be on line 2",
      );
    });
    ...
    test("Should handle multiple matchers with different ignore patterns", () => {
      ...
      const warningMatches = allMatches.filter(
        (m) => m.matcher.type === "warning",
      );
    ...
    test("Should handle complex regex patterns in ignore pattern", () => {
      ...
      assert.strictEqual(
        allMatches.length,
        3,
        "Should find 3 matches (ignoring test/spec/flaky errors)",
      );
      assert.strictEqual(
        allMatches[0].line,
        1,
      ...
      assert.strictEqual(
        allMatches[1].line,
        5,
        "Second match should be on line 5",
      );
      assert.strictEqual(
        allMatches[2].line,
        6,
        "Third match should be on line 6",
      );
    });
    ...
    test("Should handle multiline patterns in ignore pattern", () => {
      ...
      assert.strictEqual(
        allMatches.length,
        2,
        "Should find 2 matches (ignoring flaky line)",
      );
    });
  });
  ...
  suite("Configuration Validation", () => {
    test("Should validate valid ignore pattern", () => {
      ...
      assert.ok(
        result.isValid,
        "Config with valid ignore pattern should be valid",
      );
      assert.strictEqual(
    ...
    test("Should reject invalid ignore pattern regex", () => {
      ...
      assert.ok(
        !result.isValid,
        "Config with invalid ignore pattern should be invalid",
      );

      const ignorePatternError = result.errors.find((e) =>
        e.path.includes("ignorePattern"),
      );
      assert.ok(
        ignorePatternError,
        "Should have ignore pattern validation error",
      );
      assert.ok(
        ignorePatternError.message.includes("Invalid ignore regex pattern"),
        "Error message should mention invalid ignore regex",
      );
    });
    ...
    test("Should validate empty ignore pattern", () => {
      ...
      assert.ok(
        result.isValid,
        "Config with empty ignore pattern should be valid",
      );
      assert.strictEqual(
        result.errors.length,
        0,
    ...
    test("Should validate matcher without ignore pattern", () => {
      ...
      assert.ok(
        result.isValid,
        "Config without ignore pattern should be valid",
      );
      assert.strictEqual(
        result.errors.length,
        0,
  ...
  suite("Compilation Error Handling", () => {
    test("Should handle invalid ignore pattern gracefully during compilation", () => {
      ...
      assert.strictEqual(
        ...
        1,
        "Should compile matcher despite invalid ignore pattern",
      );
      assert.ok(compiledMatchers[0].regex, "Should have main regex compiled");
      assert.ok(
        !compiledMatchers[0].ignoreRegex,
        "Should not have ignore regex due to invalid pattern",
      );
    });
    ...
    test("Should work normally when ignore pattern compilation fails", () => {
      ...
      assert.strictEqual(
        allMatches.length,
        2,
        "Should match all lines when ignore pattern fails",
      );
    });
  });
  ...
  suite("Real-world Examples", () => {
    test("Should handle Chromium log example from config file", () => {
      ...
      assert.strictEqual(
        allMatches.length,
        4,
        "Should find 4 matches (ignoring 2 flaky errors)",
      );
      ...
      assert.ok(
        !matchedLines.includes(3),
        "Should not match line 3 (flaky ERROR)",
      );
      ...
      assert.ok(
        !matchedLines.includes(7),
        "Should not match line 7 (flaky ERROR)",
      );
    });
  });
});

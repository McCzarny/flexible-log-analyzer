...
suite('Language Mode Change Tests', () => {
	...
	suite('Configuration Parsing', () => {
		test('Should parse changeLanguageMode flag from detector when true', async () => {
			const configPath = path.join(__dirname, '../fixtures/config-with-language-mode/chromium-with-mode.yaml');
		...
		test('Should parse changeLanguageMode flag from detector when false', async () => {
			const configPath = path.join(__dirname, '../fixtures/config-without-language-mode/chromium-without-mode.yaml');
			const configUri = vscode.Uri.file(configPath);
	...
	suite('Language Mode Detection Logic', () => {
		...
		test('Should not match non-Chromium log format', async () => {
			...
			const testFilePath = path.join(__dirname, '../fixtures/non-chromium-log.log');
			const config = await configManager.getConfigForFile(testFilePath);
			...
			assert.ok(!config || config.name !== 'Test Chromium Config', 'Should not match non-Chromium format');
		});
	});
	...
	suite('Language Mode Change Integration', () => {
		...
		test('Should not change language mode when detector matches but flag is false', async () => {
			...
			const testFilePath = path.join(__dirname, '../fixtures/different-format-log.txt');
			
			// Open the document first
			const document = await vscode.workspace.openTextDocument(testFilePath);
			const initialLanguageId = document.languageId;
			outputChannel.appendLine(`Initial language mode for ${path.basename(testFilePath)}: ${initialLanguageId}`);
			...
			assert.ok(!config, 'Should not find configuration for non-matching file content');
			...
			const finalLanguageId = document.languageId;
			outputChannel.appendLine(`Final language mode for ${path.basename(testFilePath)}: ${finalLanguageId}`);
			assert.strictEqual(finalLanguageId, initialLanguageId, 'Document language mode should not change');
			assert.notStrictEqual(finalLanguageId, 'log', 'Document should not be in log mode');
		});
		...
		test('Should not change language mode when changeLanguageMode flag is explicitly false', async () => {
			...
			const testFilePath = path.join(__dirname, '../fixtures/chromium-log-with-ext.log');
			...
			assert.ok(config, 'Should find configuration');
			assert.ok(config.detector, 'Config should have detector');
			assert.strictEqual(config.detector.changeLanguageMode, false, 'Config should have changeLanguageMode=false');
			...
			const finalLanguageId = document.languageId;
			outputChannel.appendLine(`Final language mode for ${path.basename(testFilePath)}: ${finalLanguageId}`);
			assert.strictEqual(finalLanguageId, initialLanguageId, 'Document language mode should not change when flag is false');
			assert.notStrictEqual(finalLanguageId, 'log', 'Document should not be in log mode when flag is false');
		});
		...
		test('Should not change language mode for file pattern matches (non-detector)', async () => {
			...
			(configManager as any).configs.set('test-config-file-pattern', testConfig);
			...
			const finalLanguageId = document.languageId;
			assert.strictEqual(finalLanguageId, initialLanguageId, 'Language mode should not change for file pattern matches');
		});
	});
	...
	suite('Edge Cases', () => {
		test('Should handle missing detector gracefully', async () => {
			...
			assert.ok(!config || config.name !== 'Test Config No Detector', 'Should not match without detector');
		});

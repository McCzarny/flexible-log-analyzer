  ...
  suite("Log Parsing Tests", () => {
    ...
    test("Should parse chromium.yaml configuration correctly", async () => {
      ...
      const testConfigPath = path.join(
        __dirname,
        "../../../testdir/.logconfig/chromium.yaml",
      );
      ...
      try {
        ...
        assert.strictEqual(
          config.name,
          "Chromium log configuration",
          "Name should match",
        );
        assert.strictEqual(
          config.description,
          "Basic configuration for Chromium log file analysis",
        ...
        const errorMatcher = config.matchers.find(
          (m) => m.name === "Error Messages",
        );
        ...
        const warningMatcher = config.matchers.find(
          (m) => m.name === "Warning Messages",
        );
        ...
        const fatalMatcher = config.matchers.find(
          (m) => m.name === "Fatal/Critical",
        );
    ...
    test("Should match patterns in Chromium log file", async () => {
      ...
      assert.ok(
        ...
        "Should find at least 2 error matches",
      );

      const warningMatches = results.filter(
        (r) => r.matcher.type === "warning",
      );
      assert.ok(
        warningMatches.length >= 1,
        "Should find at least 1 warning match",
      );
      ...
      assert.ok(
        ...
        "Should have critical severity matches",
      );
      assert.ok(highMatches.length > 0, "Should have high severity matches");
      assert.ok(
        mediumMatches.length > 0,
        "Should have medium severity matches",
      );
    });

    test("Should detect Chromium log format from file content", async () => {
      const chromiumLogLine =
        "[91167:11504282:0816/095805.977857:ERROR:chrome/browser/mac/code_sign_clone_manager.mm:98] error removing quarantine attribute";
      ...
      assert.ok(
        regex.test(chromiumLogLine),
        "Chromium log line should match detector pattern",
      );
      ...
      for (const line of nonChromiumLines) {
        assert.ok(
          !regex.test(line),
          `Line "${line}" should not match Chromium pattern`,
        );
      }
    });

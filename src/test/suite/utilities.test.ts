...
suite("Utility Functions Tests", () => {
  ...
  suite("Severity and Type Utilities", () => {
    test("Should handle severity priority calculation", () => {
      ...
      assert.strictEqual(
        unknownPriority,
        5,
        "Unknown severity should have lowest priority",
      );
    });
    ...
    test("Should provide icons for different severities", () => {
      ...
      assert.strictEqual(
        mockTreeView.getIconForSeverity("critical"),
        "$(error)",
        "Critical should use error icon",
      );
      assert.strictEqual(
        mockTreeView.getIconForSeverity("high"),
        "$(error)",
        "High should use error icon",
      );
      assert.strictEqual(
        mockTreeView.getIconForSeverity("medium"),
        "$(warning)",
        "Medium should use warning icon",
      );
      assert.strictEqual(
        mockTreeView.getIconForSeverity("low"),
        "$(info)",
        "Low should use info icon",
      );
      assert.strictEqual(
        mockTreeView.getIconForSeverity("unknown"),
        "$(circle-outline)",
        "Unknown should use default icon",
      );
    });
    ...
    test("Should provide colors for different severities", () => {
      ...
      assert.strictEqual(
        mockTreeView.getColorForSeverity("critical"),
        "#8B0000",
        "Critical should be dark red",
      );
      assert.strictEqual(
        mockTreeView.getColorForSeverity("high"),
        "#FF4444",
        "High should be red",
      );
      assert.strictEqual(
        mockTreeView.getColorForSeverity("medium"),
        "#FFA500",
        "Medium should be orange",
      );
      assert.strictEqual(
        mockTreeView.getColorForSeverity("low"),
        "#0088FF",
        "Low should be blue",
      );
      assert.strictEqual(
        mockTreeView.getColorForSeverity("unknown"),
        "#666666",
    ...
    test("Should capitalize type names correctly", () => {
      ...
      assert.strictEqual(
        mockTreeView.capitalizeType("error"),
        "Error",
        "Should capitalize error",
      );
      assert.strictEqual(
        mockTreeView.capitalizeType("warning"),
        "Warning",
        "Should capitalize warning",
      );
      assert.strictEqual(
        mockTreeView.capitalizeType("info"),
        "Info",
        "Should capitalize info",
      );
      assert.strictEqual(
        mockTreeView.capitalizeType("debug"),
        "Debug",
        "Should capitalize debug",
      );
      assert.strictEqual(
        mockTreeView.capitalizeType(""),
        "",
        "Should handle empty string",
      );
      assert.strictEqual(
        mockTreeView.capitalizeType("a"),
        "A",
  ...
  suite("Text Processing Utilities", () => {
    test("Should create preview text from lines", () => {
      ...
      const longLine =
        "This is a very long line that exceeds the maximum preview length and should be truncated with ellipsis at the end";

      assert.strictEqual(
        mockTreeView.createPreview(shortLine),
        shortLine,
      ...
      assert.ok(
        preview.includes("This is a very long line"),
        "Preview should contain beginning of line",
      );
    });
    ...
    test("Should extract filename from path correctly", () => {
      ...
      assert.strictEqual(
        mockTreeView.getFileName("/path/to/file.log"),
        "file.log",
        "Should extract filename from Unix path",
      );
      assert.strictEqual(
        mockTreeView.getFileName("C:\\path\\to\\file.log"),
        "file.log",
        "Should handle Windows paths",
      );
      assert.strictEqual(
        mockTreeView.getFileName("file.log"),
        "file.log",
        "Should handle filename without path",
      );
      assert.strictEqual(
        mockTreeView.getFileName("/path/to/"),
        "to",
        "Should handle path ending with separator",
      );
      assert.strictEqual(
        mockTreeView.getFileName(""),
        "",
        "Should handle empty string",
      );
    });
    ...
    test("Should handle file size parsing", () => {
      ...
      assert.strictEqual(
        mockPatternMatcher.parseFileSize("100B"),
        100,
        "Should parse bytes",
      );
      assert.strictEqual(
        mockPatternMatcher.parseFileSize("1KB"),
        1024,
        "Should parse kilobytes",
      );
      assert.strictEqual(
        mockPatternMatcher.parseFileSize("1MB"),
        1024 * 1024,
        "Should parse megabytes",
      );
      assert.strictEqual(
        mockPatternMatcher.parseFileSize("1GB"),
        1024 * 1024 * 1024,
        "Should parse gigabytes",
      );
      assert.strictEqual(
        mockPatternMatcher.parseFileSize("2.5MB"),
        Math.floor(2.5 * 1024 * 1024),
        "Should parse decimal values",
      );
      assert.strictEqual(
        mockPatternMatcher.parseFileSize("invalid"),
        0,
  ...
  suite("Context Extraction", () => {
    test("Should extract context around matches", () => {
      ...
      const line =
        "2025-08-16 10:30:00 [ERROR] Database connection failed due to network timeout";
      const errorIndex = line.indexOf("ERROR");
      const errorLength = 5;

      const context = mockPatternMatcher.extractContext(
        line,
        errorIndex,
        errorLength,
      );
      ...
      assert.ok(
        context.includes("10:30:00"),
        "Context should include some preceding text",
      );
      assert.ok(
        context.includes("Database"),
        "Context should include some following text",
      );
    });
    ...
    test("Should handle edge cases in context extraction", () => {
      ...
      const context3 = mockPatternMatcher.extractContext(
        longLine,
        longErrorIndex,
        5,
      );
      assert.ok(
        context3.includes("ERROR"),
        "Should include match in long line",
      );
      assert.ok(
        context3.startsWith("..."),
        "Should truncate beginning of long context",
      );
      assert.ok(
        context3.endsWith("..."),
        "Should truncate end of long context",
      );
    });
  });
});

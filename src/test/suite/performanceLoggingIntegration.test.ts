...
suite('Performance Logging Integration Tests', () => {
  ...
  test('Should create performance logger in PatternMatcher', () => {
    ...
    assert.ok(typeof patternMatcher.getPerformanceMetrics === 'function',
  ...
  test('Should return performance metrics with correct structure', () => {
    ...
    assert.ok(typeof metrics.configLoadTime === 'number', 'Should have configLoadTime');
    assert.ok(typeof metrics.patternCompileTime === 'number', 'Should have patternCompileTime');
    assert.ok(typeof metrics.fileAnalysisTime === 'number', 'Should have fileAnalysisTime');
    assert.ok(typeof metrics.uiUpdateTime === 'number', 'Should have uiUpdateTime');
    assert.ok(typeof metrics.totalTime === 'number', 'Should have totalTime');
    assert.ok(typeof metrics.fileSizeBytes === 'number', 'Should have fileSizeBytes');
  ...
  test('Should log performance data during pattern compilation', () => {
    ...
    const compilationMessages = loggedMessages.filter(msg => 
      msg.includes('Compiled') && msg.includes('matchers'));
    assert.ok(compilationMessages.length > 0, 'Should log pattern compilation results');
  });
  ...
  test('Should log performance data during file analysis', async () => {
    ...
    const result = await patternMatcher.analyzeFile('/test/file.log', testConfig, testContent);

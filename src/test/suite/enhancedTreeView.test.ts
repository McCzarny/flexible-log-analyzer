...
suite('EnhancedTreeView Cache Management Tests', () => {
    ...
    suite('Cache Validation', () => {
        test('Should validate cache based on checksum', () => {
            ...
            const documentChecksum = ChecksumUtils.calculateDocumentChecksum(documentContent);
            const checksum1 = 'abc123';
            const checksum2 = 'def456';

            // Initially no cache
            assert.ok(!treeView.isCacheValid(filePath, documentContent, checksum1), 'Should not be valid when no cache exists');
            ...
            const result: AnalysisResult = {
                ...
                totalLines: 100,
                matches: [],
                config: { version: '1.0', name: 'Test Config', matchers: [], checksum: checksum1, filePath: '/in-memory-config.yaml' },
                analysisTime: 50,
            ...
            assert.ok(treeView.isCacheValid(filePath, documentContent, checksum1), 'Should be valid with matching checksum');
            // Should be valid with same checksum
            assert.ok(!treeView.isCacheValid(filePath, documentContent, checksum2), 'Should not be valid with different checksum');

            // Should not be valid with different document checksum
            assert.ok(!treeView.isCacheValid(filePath, 'differentDocChecksum', checksum1), 'Should not be valid with different document checksum');
        });
        ...
        test('Should invalidate cache when checksum changes', () => {
            ...
            const cachedResult1 = treeView.getCachedResult(filePath, documentContent, checksum1);
            ...
            const cachedResult2 = treeView.getCachedResult(filePath, documentContent, checksum2);
            ...
            const cachedResult3 = treeView.getCachedResult(filePath, documentContent, checksum1);
    ...
    suite('Configuration Path Invalidation', () => {
        test('Should invalidate cache by configuration path', () => {
            ...
            const result1: AnalysisResult = {
                ...
                totalLines: 100,
                matches: [],
                config: { version: '1.0', name: 'Workspace Config', matchers: [], checksum: 'workspace_config_v1', filePath: configPath },
                analysisTime: 50,
                summary: { totalMatches: 0, matchesBySeverity: {} as any, matchesByType: {} as any },
                badgeCount: 0,
                fileLinks: [],
            ...
            const result2: AnalysisResult = {
                ...
                totalLines: 200,
                matches: [],
                config: { version: '1.0', name: 'Workspace Config', matchers: [], checksum: 'workspace_config_v2', filePath: configPath },
                analysisTime: 75,
                summary: { totalMatches: 0, matchesBySeverity: {} as any, matchesByType: {} as any },
                badgeCount: 0,
                fileLinks: [],
            ...
            const result3: AnalysisResult = {
                ...
                totalLines: 150,
                matches: [],
                config: { version: '1.0', name: 'Other Config', matchers: [], checksum: 'other_config_v1', filePath: '/in-memory-config.yaml' },
                analysisTime: 60,
                summary: { totalMatches: 0, matchesBySeverity: {} as any, matchesByType: {} as any },
                badgeCount: 0,
                fileLinks: [],
            ...
            assert.ok(treeView.isCacheValid(file1, documentContent, result1.config.checksum), 'File1 should be cached');
            assert.ok(treeView.isCacheValid(file2, documentContent, result2.config.checksum), 'File2 should be cached');
            assert.ok(treeView.isCacheValid(file3, documentContent, result3.config.checksum), 'File3 should be cached');
            ...
            assert.ok(!treeView.isCacheValid(file1, documentContent, 'workspace_config_v1'), 'File1 should not be cached');
            assert.ok(!treeView.isCacheValid(file2, documentContent, 'workspace_config_v1'), 'File2 should not be cached');
            assert.ok(treeView.isCacheValid(file3, documentContent, 'other_config_v1'), 'File3 should still be cached');
        });
    ...
    suite('Result Management', () => {
        test('Should update results and maintain cache', () => {
            ...
            const result: AnalysisResult = {
                ...
                totalLines: 100,
                matches: [],
                config: { version: '1.0', name: 'Test Config', matchers: [], checksum: configChecksum, filePath: '/in-memory-config.yaml' },
                analysisTime: 50,
                summary: { totalMatches: 0, matchesBySeverity: {} as any, matchesByType: {} as any },
                badgeCount: 0,
                fileLinks: [],
            ...
            assert.ok(!treeView.isCacheValid(filePath, kEmptyDocument, configChecksum), 'Should not be cached initially');

            // Update results
            treeView.updateResults(result, kEmptyDocument);

            // Should now be cached
            assert.ok(treeView.isCacheValid(filePath, kEmptyDocument, configChecksum), 'Should be cached after update');

            // Should be able to retrieve cached result
            const cachedResult = treeView.getCachedResult(filePath, kEmptyDocument, configChecksum);
        ...
        test('Should remove specific file results', () => {
            ...
            const result1: AnalysisResult = {
                ...
                totalLines: 100,
                matches: [],
                config: { version: '1.0', name: 'Test Config', matchers: [], checksum: 'test_checksum', filePath: '/in-memory-config.yaml' },
                analysisTime: 50,
                summary: { totalMatches: 0, matchesBySeverity: {} as any, matchesByType: {} as any },
                badgeCount: 0,
                fileLinks: [],
            ...
            assert.ok(treeView.isCacheValid(filePath1, kEmptyDocument, 'test_checksum'), 'File1 should be cached');
            assert.ok(treeView.isCacheValid(filePath2, kEmptyDocument, 'test_checksum'), 'File2 should be cached');

            // Remove file1
            treeView.removeResults(filePath1);

            // File1 should be removed, file2 should remain
            assert.ok(!treeView.isCacheValid(filePath1, kEmptyDocument, 'test_checksum'), 'File1 should be removed');
            assert.ok(treeView.isCacheValid(filePath2, kEmptyDocument, 'test_checksum'), 'File2 should remain cached');
        });
        ...
        test('Should clear all results', () => {
            ...
            // Both should be cached
            assert.ok(treeView.isCacheValid(filePath1, kEmptyDocument, configChecksum), 'File1 should be cached');
            assert.ok(treeView.isCacheValid(filePath2, kEmptyDocument, configChecksum), 'File2 should be cached');

            // Clear all results
            treeView.clearResults();

            // Both should be removed
            assert.ok(!treeView.isCacheValid(filePath1, kEmptyDocument, configChecksum), 'File1 should be cleared');
            assert.ok(!treeView.isCacheValid(filePath2, kEmptyDocument, configChecksum), 'File2 should be cleared');
        });
    });
});